...
class OrderNewRepository @Inject constructor(
    ...
    fun getOrdersFlow(): Flow<List<OrderSummary>> {
        val rows = orderLocalDataSource.getOrdersFlow()
    ...
    fun getOrderFlowById(id: String): Flow<OrderNew> {
        val rows = orderLocalDataSource.getOrderFlowById(id)
        val orders = rows.map { rows ->
            OrderNew(
                id = rows[0].id,
    ...
    suspend fun updateOrderStatus(id: String, status: String) {
        orderLocalDataSource.updateOrderStatus(id, status)
    }
